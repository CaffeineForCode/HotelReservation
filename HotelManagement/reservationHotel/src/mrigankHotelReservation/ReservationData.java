...
public class ReservationData{
    ...
    public ReservationData(String[][] resData, Hotels hot){
        ...
        for(int i = 1; i< resData.length; i++){
            ...
            if(count == 0){
                ...
                String dateFrom = resData[i][2];
                int nights = Integer.parseInt(resData[i][4]);
                int rooms = Integer.parseInt(resData[i][5]);
                String roomType = resData[i][6];
                int adults = Integer.parseInt(resData[i][7]);
                int children = Integer.parseInt(resData[i][8]);
                boolean breakfast = Boolean.parseBoolean(resData[i][9]);
                double deposit = Double.parseDouble(resData[i][10]);
    ...
    */
    public boolean cancellations(int num, Hotels hot, StartHere startHere){
        GregorianCalendar greg = new GregorianCalendar();
        boolean flip = false;
        ...
        startHere.getCheckData().getCancellations().add(cancelled);
        startHere.getRead().writeCancellations(hot, startHere.getCheckData());
        for(int i = 0; i < list.size(); i++){
            if(list.get(i).getNumber() == num){
                if((list.get(i).getDateFrom().getTimeInMillis() - greg.getTimeInMillis()) > 172800000 ){
                    list.remove(i);
